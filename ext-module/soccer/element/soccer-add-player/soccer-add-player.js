(async () => {
     ...
     * @polymer
     */
    class SoccerAddPlayer extends LocalizeMixin(ServiceInjectorMixin(PolymerElement)) {
        ...
        submit(evt) {
            ...
            if (!event != 'update') {
                this.player = new SoccerPlayerEntity();
            }
        }

        /**
         * 
         * @param SoccerPlayerEntity} player 
         * @returns {SoccerPlayerEntity}
         */
        _clonePlayer(player) {
            let clone = new SoccerPlayerEntity();
            clone.id = player.id;
    ...
    window.customElements.define('soccer-add-player', SoccerAddPlayer);
})()
